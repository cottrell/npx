...
def sum_at(a, indices, minlength):
    ...
    if len(a.shape) < len(indices.shape):
        raise RuntimeError(
            f'a.shape = {a.shape}, indices.shape = {indices.shape}, but len(a.shape) >= len(indices.shape) is required.'
            )
    m = len(indices.shape)
    ...
    return np.array([np.bincount(indices, weights=a[:, k], minlength=
        minlength) for k in range(a.shape[1])]).T.reshape(out_shape)
